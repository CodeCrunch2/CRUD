...
public class JavaIOProjectRepositoryImpl implements ProjectRepository {
    ...
    static {
        if (MainView.isWindows()) path = "src\\main\\resources\\projects.txt";
        else path = "src/main/resources/projects.txt";
    }
    @Override
    ...
    public Set<Developer> getDevelopersSet(String[] strArr) throws DeveloperNotExistException {
        ...
        for (int i = 1; i < strArr.length - 1; i++) {
            if(!strArr[i].equals("null")) developerSet.add(developerController.getByID(strArr[i]));
        }
        return developerSet;
    }
